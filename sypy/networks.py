...
class Network:
	...
	def reset_attack_edges(self, num_edges):
		self.graph = self.__setup_network_graph()
		self.is_stitched = False
		self.attack_edges = []
		self.random_pair_stitch(num_edges)
	...
	def visualize(self, file_name=None, file_format="pdf", dim=2, k=None, pos=None, fixed=None, iterations=50, weight='weight', scale=1.0, labeled=False):
		layout = nx.spring_layout(self.graph.structure, dim=dim, k=k, pos=pos, fixed=fixed, iterations=iterations, weight=weight, scale=scale)
