...
class SimpleDetectorBenchmark:
	...
	def run(self):
		...
		for value in self.values:
			...
			results[value] = self.detector.classify()
...
class SimpleAttackEdgesDetectorsBenchmark:
	"""
	Same as above but runs the attacking / detecting procedure a couple of 
	times for each ammount of attack edges. To get mean and variance in the ende.
	"""
	def __init__(self, detector_name, network, threshold, values=None, attack_edge_values=None, num_sample=10, kwargs=None):
		...
		if not self.values:
			self.values = [i/10.0 for i in xrange(0,11)]
		self.attack_edge_values = attack_edge_values
		if not self.attack_edge_values:
			left = len(self.multi_benchmark.network.left_region.graph.nodes())
			right = len(self.multi_benchmark.network.right_region.graph.nodes())
			max_edges = 2 * (left + right)
			step_size = max_edges / 25
			self.attack_edge_values =  [1] + [ i*step_size for i in range(1, 25) ]
			print 'default attack_edge_values:', self.attack_edge_values
		
	
	def run(self):
		num_values = len(self.attack_edge_values)
		auc_vals = np.zeros((num_values, self.num_sample))
		for j, num_attack_edges in enumerate(self.attack_edge_values):
			print 'number attacking edges:', num_attack_edges
			for i in range(self.num_sample):
				#print 'run (', i, '/', self.num_sample, ')'
				self.network.reset_attack_edges(num_attack_edges)
				detector = self.detector_name(self.network, verifiers=self.network.known_honests, **self.kwargs if self.kwargs else {})
				simple_benchmark = SimpleDetectorBenchmark(detector, self.threshold, values=self.values)
				simple_benchmark.run()
				auc_vals[j, i] = simple_benchmark.curve["auc"]
		#print 'auc_vals', auc_vals
		return np.mean(auc_vals, axis=1), np.std(auc_vals, axis=1)		
