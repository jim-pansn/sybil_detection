...
class GirvanNewmanCommunityDetector(BaseDetector):
	...
	def __construct_dendogram(self, structure, current_level):
		...
		sub_structures = list(nx.connected_component_subgraphs(structure))
...
class SybilRankDetector(BaseSybilDetector):
	...
	def detect(self):
		...
		#print 'numer iterations:', num_iterations
		...
		#print 'network_trust:', network_trust
		self.ranked_trust = self.__normalize_and_rank_network_trust(network_trust)
		#print '(SybilRank) ranked:', self.ranked_trust

		# classify
		pivot_mark = (int)(self.pivot * len(self.ranked_trust))
		verified_honests = [
			honest_node for honest_node, trust in self.ranked_trust[pivot_mark:]
		]
